...
// 加载
func (u QiniuUploader) Upload(file multipart.File, fileName string) (map[string]interface{}, error) {
	...
	fmt.Println(size)
	//最大图片尺寸验证
	if size >= int64(config.Qiniu.ImageMaxSize)*1024*1024 {
		return map[string]interface{}{}, errors.New("上传大小不能超过" + strconv.Itoa(int(config.Qiniu.ImageMaxSize)) + "M")
	}
...
func (u QiniuUploader) VideoUpload(file multipart.File, fileName string) (map[string]interface{}, error) {
	config := util.GetConfig()
	var (
		ret  PutRet
		size int64
	...
	fmt.Println(size)
	//最大图片尺寸验证
	if size >= int64(config.Qiniu.VideoMaxSize)*1024*1024 {
		return map[string]interface{}{}, errors.New("上传大小不能超过" + strconv.Itoa(int(config.Qiniu.VideoMaxSize)) + "M")
	}
	putPolicy := storage.PutPolicy{
		Scope:      config.Qiniu.Bucket,
		ReturnBody: `{"key":"$(key)","hash":"$(etag)","fsize":$(fsize),"bucket":"$(bucket)","name":"$(x:name)"}`,
